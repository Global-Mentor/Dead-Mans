        ...
        if (useRussian)
        {
            lines.AddRange(
                [
                    ...
                    "  // Поле answers (необязательно): список альтернативных корректных ответов.",
                    "  // Первое значение в answers считается основным ответом; пустые элементы игнорируются.",
                    ...
                    "  // - answers: необязательный список дополнительных правильных ответов.",
                    "  //   Если после очистки answers ещё есть значения, они используются, первое — основное.",
        ...
        else
        {
            lines.AddRange(
                [
                    ...
                    "  // Optional field \"answers\": list of alternate correct answers.",
                    "  // The first item in answers is treated as primary; empty items are ignored.",
                    ...
                    "  // - answers: optional array of additional correct answers (for other languages or alternative wording).",
                    "  //   If answers still has values after cleanup, it is used and its first item is primary.",
