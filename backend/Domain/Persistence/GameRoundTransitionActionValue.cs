...
{
    public const string StartModifierOrdering = "start_modifier_ordering";
    ...
    public static string CheckSqlAllowed { get; } =
        $"action_code IN ('{StartModifierOrdering}','{Prepare}','{Rebuild}','{BeginGameplay}',"
        + $"'{Review}','{ResumeGameplay}','{Finalize}','{TechnicalCancel}')";

    public static string CheckSqlTransitionSemantics { get; } =
        $"(action_code = '{StartModifierOrdering}' AND from_status = 'card_opened' AND to_status = 'awaiting_modifiers') OR "
        ...
        + $"(action_code = '{BeginGameplay}' AND from_status = 'preparing' AND to_status = 'in_progress') OR "
        ...
        + $"(action_code = '{TechnicalCancel}' AND from_status IN ('card_opened','awaiting_modifiers','preparing','in_progress','reviewing_results') AND to_status = 'cancelled')";
}
