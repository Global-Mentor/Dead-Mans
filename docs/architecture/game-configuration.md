...
В черновик можно прикрепить только глобально включённые и не удалённые вопросы.
Список выбора запрашивает каталог с `includeDisabled=false`; глобальный каталог
сохраняет отключённые вопросы для последующего редактирования и включения.
Отключение вопроса (в том числе через редактирование или отключение категории)
и soft-delete в одной транзакции удаляют его из черновиков и повышают версию
каждого затронутого поля один раз. Сохранение, публикация и эти изменения каталога
используют общую блокировку; сохранение повторно проверяет доступность вопросов
под блокировкой. Повторное включение не восстанавливает прежние привязки.
Опубликованные игры сохраняют выбранные вопросы и их снимки.

После изменения каталога `draftChanged` обновляет черновик и список доступных
вопросов у администраторов. Несохранённые правки сохраняются, но недоступные
вопросы удаляются и из локального выбора. Проверка использует полный каталог,
независимо от поискового фильтра. Миграция `RemoveUnavailableDraftQuestions`
также очищает ранее сохранённые недоступные вопросы в черновиках.
...
- Подписи строк и колонок ограничены 24 символами и отображаются с переносом строк
  и адаптивным размером шрифта, чтобы поле оставалось читаемым на разных экранах.
