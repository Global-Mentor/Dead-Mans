...
- The round assistant renders the existing flow model as a read-only sequence inside `NativeDisclosure`. Only the existing primary action advances the round.
...
- `GameBoardLayout` centers the cards themselves, accounting for the row-price gutter, and keeps the status aligned with them. It measures the rendered field and anchors the compact team queue at the left edge of the available region only when the full panel fits clear of edge controls and row labels. The team list scrolls within the field height; otherwise `TeamQueuePanel` uses its existing modal trigger. The same search, grouped teams and loading/error states serve both presentations.
- `/panel/game-round` is a distinct view of the active game, linked directly after Board in the primary navigation. `RoundOverview` reads the authoritative active round, board cell and modifier state; the modifier drawer reuses the existing feature controls. Opening a card does not redirect anyone from the board. Any user can open the round manually from its open card, the phase control or the primary navigation; other revealed cards retain their preview. `GameQuizDrawer` follows players on either game view, including between rounds, and reuses the existing answer card. Modifier ordering and an open quiz cannot display their drawers simultaneously.
...
- `PlayerQuizCard` composes `CurrentQuizCard`; `useSubmitQuizAnswer` shares pending submissions and cache refresh between the quiz page and game drawers. A question drawer retains the result when its session closes and resets only for a different question. Expired questions disable answers while the common query waits for the server result.
