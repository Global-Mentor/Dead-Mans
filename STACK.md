...
- **Entity Framework Core**
  - ORM для работы с SQL базой (SQL Server или PostgreSQL), миграции, типобезопасные запросы.
...
- **ASP.NET Identity + роли**
  - Базовая система пользователей, ролей и прав (стример, модератор, зритель и т.д.).

- **OAuth2 Twitch**
  - Авторизация через Twitch, чтобы привязывать пользователей к их Twitch‑аккаунтам.

- **ImageSharp / SkiaSharp**
  - Серверная генерация составных изображений (оверлеи, итоги игр, карточки).
...
- **CI/CD (GitHub Actions)**
  - Автоматическая сборка, тесты и деплой фронта/бэкенда при пуше в `main` или релизные ветки.
