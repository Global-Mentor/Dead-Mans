import type { GameControlState, GamePhase } from '../contracts/index.ts'
...
export async function nextRound(): Promise<GameControlState> {
  ...
  setState({ phase: (next >= state.totalRounds ? 'finished' : state.phase) as GamePhase, currentRound: next })
  return getControlState()
}
