...
for (const viewport of [
  ...
  test(`question form is compact and keeps the correct answer first at ${viewport.width}px`, async ({
    ...
    await dialog.getByLabel('Вопрос', { exact: true }).fill('Какой город является столицей Польши?')
    await dialog.getByLabel('Правильный ответ', { exact: true }).fill('Варшава')
    ...
    expect((await remove.boundingBox())!.width).toBeLessThanOrEqual(40)
    await page.evaluate(() => document.fonts.ready)
    ...
    await expect(dialog.getByLabel('Правильный ответ', { exact: true })).toHaveValue('Варшава')
